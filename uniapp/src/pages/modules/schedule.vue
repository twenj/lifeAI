  ...
  <view class="page">
    ...
    <view class="body">
      <view class="form-card">
        <view class="form-title">添加日程</view>
        ...
        <view class="row">
          <picker mode="time" :value="draft.startTime" @change="draft.startTime = $event.detail.value"><view class="field half">开始：{{ draft.startTime }}</view></picker>
        ...
        <view class="primary-button" @tap="save">保存日程</view>
      </view>
      ...
      <view v-for="item in items" :key="item.id" class="schedule-card" :class="{ done: item.completed }">
        ...
        <view class="actions"><view class="complete" @tap="toggle(item)">{{ item.completed ? '已完成' : '完成' }}</view><view class="delete" @tap="remove(item)">删除</view></view>
      </view>
    </view>
  </view>
</template>
...
const save = async () => {
  ...
  try {
    const created = await backendApi.createSchedule({ title: draft.title.trim(), startAt: toIso(draft.date, draft.startTime), endAt: toIso(draft.date, draft.endTime), repeat: draft.repeat, reminderMinutes: draft.reminderMinutes, notes: draft.notes.trim() || undefined })
    scheduleLocalReminder(created)
    draft.title = ''; draft.notes = ''; await load(); uni.showToast({ title: '保存成功', icon: 'success' })
...
.complete, .delete { padding: 8rpx; font-size: 24rpx; }
